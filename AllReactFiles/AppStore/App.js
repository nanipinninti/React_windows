...
class App extends Component{
  state = {activeTab:tabsList[0].tabId,ip:""}
  fileterApps = (inp)=>{
    const {activeTab} = this.state
    const filteredapps = appsList.filter(obj=>(
      obj.category === activeTab && obj.appName.toLowerCase().includes(inp.toLowerCase())
    ))
    return filteredapps
  }
  updateTab = (tabid)=>{
  ...
  render(){
    const {activeTab,ip} = this.state
    const filiteredappps = this.fileterApps(ip)
    return(
      <div className='container'>
        <div className='innercontainer'>
          <h1 className='heading'>App Store</h1>
          <div className='serachtab'>
            <input type='search' placeholder='Search' className='search' onChange={this.enteringinput} />
