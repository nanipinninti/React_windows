...
class MathGame extends Component{
    ...
    componentDidMount(){
        this.intervalId = setInterval(this.remaingtime,1000)
    }
    ...
    remaingtime = ()=>{
        const {elapsedtime,isplaying} = this.state
        if (!isplaying){
            return
        }
        if (elapsedtime >0){
    ...
    togglePlayingMode = ()=>{
        this.setState({
            ...
            score : this.state.isplaying ? this.state.score : 0 ,
            elapsedtime: this.state.isplaying? this.state.elapsedtime:60
        })
    }
    activeTab = (id)=>{
        this.setState({
    ...
    filteredImagesList = ()=>{
        ...
        const filteredimageslist = imagesList.filter(obj=>(activeTab===obj.category))
        return filteredimageslist
    }
    setRandomImage = ()=>{
        const randomIndex = this.getRandomNumber(0,LENGTH-1)
        const imgSrc = imagesList[randomIndex].imageUrl
        const currId = imagesList[randomIndex].id
        this.setState({
            currImageSrc : imgSrc,
            currId : currId
        })
    ...
    gamerOverDisplay = ()=>{
        ...
        return(
            <div className='gameover-card'>
                <img className='trophy' alt='trophy-img'
                src='https://assets.ccbp.in/frontend/react-js/match-game-trophy.png'/>
                <h1 className='text'>YOUR SCORE</h1>
                <h1 className='gamerover-score'>{score}</h1>
                <button onClick={this.togglePlayingMode} className='play-again-botton'>
                    <img src='https://assets.ccbp.in/frontend/react-js/match-game-play-again-img.png '
                        alt='reset' className='reset-img img'/>
                    <h3 >PLAY AGAIN</h3>
                </button>
            </div>
        )
    ...
    navbar = ()=>{
        const {isplaying,score,elapsedtime} = this.state
        return (
            <div className='nav-bar'>
                <img src='https://assets.ccbp.in/frontend/react-js/match-game-website-logo.png'
                    alt="app-logo" className='web-logo'
                />
                <div className='nav-items'>
                    <h2 className='score-text'>Score : <span className='nav-text'>{score}</span></h2>
                    <img src='https://assets.ccbp.in/frontend/react-js/match-game-timer-img.png'
                        alt='timer-icon' className='timer-icon' />
                    <h1 className='nav-text'>{elapsedtime}</h1>
    ...
    playingMode = ()=>{
        const {currImageSrc,currId,imgSrc} = this.state
        const filteredimageslist = this.filteredImagesList()
        return(
            <div className='playing-mode'>
                <img src={currImageSrc} alt='image'
                    className='current-img' />
                    <ul className='tabitems-list list'>
                        {
                            tabsList.map(obj=>(
                                <TabItems 
                                    key = {obj.id}
                                    ...
                                    ActiveTabChange = {this.activeTab}
                                />
                            ))
                        }
                    </ul>
                    <ul className='thumbnails-list list'>
                        {
                            filteredimageslist.map(obj=>(
                                <Thumbnails
                                    key = {obj.id}
                                    ThumbnailDetails = {obj}
                                    ThumbnailClickCheck ={this.ThumbnailClickCheck}
                                />
                            ))
                        }
                    </ul>
    ...
    render(){
        const {currImageSrc,currId,imgSrc,isplaying} = this.state
        if (currId===-1){
            this.setRandomImage()
        }
        return(
            <div className='body'>
                ...
                <div className='container'>
                    {
                        isplaying ? this.playingMode() : this.gamerOverDisplay()
                    }
                </div>
            </div>
        )
